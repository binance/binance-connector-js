...
async function getPositionByToken() {
    try {
        const response = await client.restAPI.getPositionByToken({
            walletAddress: '0x12e32db8817e292508c34111cbc4b23340df542c',
            tokenId: '112233',
        });

        const rateLimits = response.rateLimits!;
        console.log('getPositionByToken() rate limits:', rateLimits);

        const data = await response.data();
        console.log('getPositionByToken() response:', data);
    } catch (error) {
        console.error('getPositionByToken() error:', error);
    }
}
