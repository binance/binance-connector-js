...
async function queryPnL() {
    try {
        const response = await client.restAPI.queryPnL({
            walletAddress: '0x12e32db8817e292508c34111cbc4b23340df542c',
        });

        const rateLimits = response.rateLimits!;
        console.log('queryPnL() rate limits:', rateLimits);

        const data = await response.data();
        console.log('queryPnL() response:', data);
    } catch (error) {
        console.error('queryPnL() error:', error);
    }
}
