...
async function batchRedeem() {
    try {
        const response = await client.restAPI.batchRedeem({
        ...
        console.log('batchRedeem() rate limits:', rateLimits);

        const data = await response.data();
        console.log('batchRedeem() response:', data);
    } catch (error) {
        console.error('batchRedeem() error:', error);
    }
}
