...
async function queryOrderHistory() {
    try {
        const response = await client.restAPI.queryOrderHistory({
            walletAddress: '0x12e32db8817e292508c34111cbc4b23340df542c',
        });

        const rateLimits = response.rateLimits!;
        console.log('queryOrderHistory() rate limits:', rateLimits);

        const data = await response.data();
        console.log('queryOrderHistory() response:', data);
    } catch (error) {
        console.error('queryOrderHistory() error:', error);
    }
}
