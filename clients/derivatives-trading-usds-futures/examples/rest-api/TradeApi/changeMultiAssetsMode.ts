...
async function changeMultiAssetsMode() {
    try {
        ...
        console.log('changeMultiAssetsMode() rate limits:', rateLimits);

        const data = await response.data();
        console.log('changeMultiAssetsMode() response:', data);
    } catch (error) {
        console.error('changeMultiAssetsMode() error:', error);
    }
}
