...
async function getOrderModifyHistory() {
    try {
        ...
        console.log('getOrderModifyHistory() rate limits:', rateLimits);

        const data = await response.data();
        console.log('getOrderModifyHistory() response:', data);
    } catch (error) {
        console.error('getOrderModifyHistory() error:', error);
    }
}
