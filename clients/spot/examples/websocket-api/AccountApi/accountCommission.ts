...
async function accountCommission() {
    ...
    try {
        ...
        const response = await connection.accountCommission({
            symbol: 'BNBUSDT',
        });

        const rateLimits = response.rateLimits!;
        console.log('accountCommission() rate limits:', rateLimits);

        const data = response.data;
        console.log('accountCommission() response:', data);
    } catch (error) {
        console.error('accountCommission() error:', error);
    } finally {
