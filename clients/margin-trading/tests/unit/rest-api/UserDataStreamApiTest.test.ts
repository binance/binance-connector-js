...
describe('UserDataStreamApi', () => {
    ...
    describe('keepaliveUserDataStream()', () => {
        it('should execute keepaliveUserDataStream() successfully with required parameters only', async () => {
            const params: KeepaliveUserDataStreamRequest = {
            ...
            const response = await client.keepaliveUserDataStream(params);

            await expect(response.data()).resolves.toBeUndefined();
            spy.mockRestore();
        });

        it('should execute keepaliveUserDataStream() successfully with optional parameters', async () => {
            const params: KeepaliveUserDataStreamRequest = {
                listenKey: 'listenKey_example',
            };
        ...
        it('should throw RequiredError when listenKey is missing', async () => {
            const _params: KeepaliveUserDataStreamRequest = {
                listenKey: 'listenKey_example',
            };
            const params = Object.assign({ ..._params });
            delete params?.listenKey;

            await expect(client.keepaliveUserDataStream(params)).rejects.toThrow(
                'Required parameter listenKey was null or undefined when calling keepaliveUserDataStream.'
            );
        });
        ...
        it('should throw an error when server is returning an error', async () => {
            ...
            await expect(client.keepaliveUserDataStream(params)).rejects.toThrow('ResponseError');
            spy.mockRestore();
        });
    });
