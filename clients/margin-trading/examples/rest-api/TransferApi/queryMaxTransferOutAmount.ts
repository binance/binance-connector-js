...
async function queryMaxTransferOutAmount() {
    try {
        ...
        console.log('queryMaxTransferOutAmount() rate limits:', rateLimits);

        const data = await response.data();
        console.log('queryMaxTransferOutAmount() response:', data);
    } catch (error) {
        console.error('queryMaxTransferOutAmount() error:', error);
    }
}
