...
async function queryPreventedMatches() {
    try {
        ...
        console.log('queryPreventedMatches() rate limits:', rateLimits);

        const data = await response.data();
        console.log('queryPreventedMatches() response:', data);
    } catch (error) {
        console.error('queryPreventedMatches() error:', error);
    }
}
