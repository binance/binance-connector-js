...
async function individualSymbolTickerStreams() {
    let connection:
        | DerivativesTradingCoinFuturesWebsocketStreams.WebsocketStreamsConnection
        | undefined;

    try {
        connection = await client.websocketStreams.connect();

        const stream = connection.individualSymbolTickerStreams({
            symbol: 'btcusdt',
        });
