...
async function queryIndexPriceConstituents() {
    try {
        ...
        console.log('queryIndexPriceConstituents() rate limits:', rateLimits);

        const data = await response.data();
        console.log('queryIndexPriceConstituents() response:', data);
    } catch (error) {
        console.error('queryIndexPriceConstituents() error:', error);
    }
}
