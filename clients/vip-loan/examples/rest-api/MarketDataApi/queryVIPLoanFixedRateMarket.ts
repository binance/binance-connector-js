...
async function queryVIPLoanFixedRateMarket() {
    try {
        ...
        console.log('queryVIPLoanFixedRateMarket() rate limits:', rateLimits);

        const data = await response.data();
        console.log('queryVIPLoanFixedRateMarket() response:', data);
    } catch (error) {
        console.error('queryVIPLoanFixedRateMarket() error:', error);
    }
}
